...
Monitors critical processes (Ralph Loop, Action Executor) and ensures uptime.
Implements graceful degradation logging.
...
class ProcessManager:
    ...
    def start_process(self, script_name):
        ...
        try:
            ...
            # Run with python executable
            proc = subprocess.Popen(
                [sys.executable, str(script_path)],
                cwd=str(BASE_DIR),
                shell=False
                # Ensure output is captured or redirected if needed, 
                # but for now letting it flow to stdout/stderr of watchdog
            )
